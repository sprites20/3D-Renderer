...
class Renderer(Widget):
    def __init__(self, **kwargs):
        ...
        self.scene = ObjFile(resource_find("AkulaClassAtackSubmarine_output.obj"))
        ...
        self.meshes_data = [
            {
                'file': 'AkulaClassAtackSubmarine_output.obj',
                'position': (-5, 20, -15),
                'rotation': (1, 0, 0, 0)
            },
            {
    ...
    def quaternion_to_angle_axis(self, q):
        # Calculate the angle
    ...
    def setup_bullet(self):
        ...
        for i, mesh_data in enumerate(self.meshes_data):
            ...
            meshId = p.loadURDF("AkulaClassAtackSubmarine_output.urdf", basePosition=position, baseOrientation=orientation)
    ...
    def update_glsl(self, delta):
        ...
        for mesh_data in self.meshes_data:
            ...
            # Assuming orn is a valid quaternion (qw, qx, qy, qz)
            qw, qx, qy, qz = orn
            ...
            s = math.sqrt(1 - qw**2)
            if s < 0.001:
                # If s is close to zero, the axis is not well-defined (could be any axis)
                x, y, z = 0, 0, 1  # Just set a default axis
            else:
                x = qx / s
                y = qy / s
                z = qz / s

            # Update the rotation instruction
            mesh_data["rotation"].angle = math.degrees(angle)
            mesh_data["rotation"].axis = (z, y, x)
